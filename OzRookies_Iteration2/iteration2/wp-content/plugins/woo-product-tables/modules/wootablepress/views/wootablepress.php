		...
		foreach ( $orders as $i => $column ) {
			switch ( $column['main_slug'] ) {
				...
				case 'price':
					...
					// need global variable for product to ensure compatibility with the WooCommerce Currency Switcher by WooBeWoo
					global $wtbpProduct;
					$wtbpProduct = null;
					break;
				case 'sale_dates':
					...
					case 'add_to_cart':
						...
						if ( $_product->get_stock_status() == 'outofstock' ) {
							...
							if ( in_array( 'b2b/addify_b2b.php', get_option( 'active_plugins' ) ) ) {
								...
								include  WP_PLUGIN_DIR . '/b2b/additional_classes/class_afb2b_rfq_front.php' ;
								if ( class_exists( 'Addify_Request_For_Quote_Front' ) ) {
									$addifyRequestForQuoteFront = new Addify_Request_For_Quote_Front();
								} else {
									include  WP_PLUGIN_DIR . '/b2b/woocommerce-request-a-quote/front/class-af-r-f-q-front.php' ;
									if ( class_exists( 'AF_R_F_Q_Front' ) ) {
										$addifyRequestForQuoteFront = new AF_R_F_Q_Front();
									}
								}
								if ( $addifyRequestForQuoteFront ) {
		...
		if ( $isPro ) {
			foreach ( $this->orderColumns as $column ) {
				...
				if ( array_key_exists( 'add_to_cart_popup', $column ) ) {
					...
					$table          .= ViewWtbp::getContentWithParams( 'wootablepressVariablesPopup', array( 'variationToCart' => $variationToCart ) );
				}
			}
		}
